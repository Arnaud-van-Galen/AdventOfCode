...
$ReductionSteps.Push("Add")
While ($ReductionSteps.Count -gt 0 -and $HomeWorkLine -le $HomeWork.Count) {
    $Step = $ReductionSteps.Peek() 
    [void] $ReductionSteps.Pop()
    switch ($Step) {
        "Add" {
            if ($HomeWorkLine -eq 0) {
                $Result = Adding -Pair1 $HomeWork[0] -Pair2 $HomeWork[1]
                $HomeWorkLine = 2
                $ReductionSteps.Push("Explode")
            } elseif ($HomeWorkLine -lt $HomeWork.Count) {
                $Result = Adding -Pair1 $Result -Pair2 $HomeWork[$HomeWorkLine]
                $HomeWorkLine++
                $ReductionSteps.Push("Explode")
            }
        }
...
# Correct answer = 3793 (4140 voor testdata)