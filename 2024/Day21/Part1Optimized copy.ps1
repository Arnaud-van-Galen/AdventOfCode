...
function StepCalculator([string]$From, [string]$To) {
	...
	$AllDirPad3Paths = @()
	foreach ($AllDirPad2Path in $AllDirPad2Paths) {
		# $AllDirPad2Path = 'v<<A>>^A<A>AvA<^AA>A<vAAA>^A'
		$DirPad3Paths = [array]::CreateInstance([array], $AllDirPad2Path.Length)
		$AllDirPad2Path = "A" + $AllDirPad2Path
		for ($i = 0; $i -lt $AllDirPad2Path.Length-1; $i++) {
			$DirPad3Paths[$i] = GraphSearcher -Start $AllDirPad2Path[$i] -Target $AllDirPad2Path[$i+1] -Graph $DirPad
		}
		$DirPad3Paths = ArrayUnpacker -NestedArray $DirPad3Paths
		$AllDirPad3Paths += $DirPad3Paths
		Write-Host $AllDirPad2Path
	}
	# Write-Host (($AllDirPad2Paths.ForEach{$_.Length} | Measure-Object -Minimum).Minimum)
