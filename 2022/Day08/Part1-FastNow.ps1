...
$Data_Slow = Get-Content -Path $PSScriptRoot\DataDemo.txt -ErrorAction Stop
# $Data_Slow = Get-Content -Path $PSScriptRoot\Data.txt -ErrorAction Stop
$RowCount = $Data_Slow.Count
$ColumnCount = $Data_Slow[0].Length
[int[]] $Data = [System.Collections.ArrayList]::New()  $Data_Slow.ForEach{$_.ToCharArray().foreach{[int]$_.tostring()}}
