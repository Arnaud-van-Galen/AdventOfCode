...
$AbyssRow = ($occupiedPositions.Keys.ForEach{ $_.split(",")[1] } | Measure-Object -Maximum).Maximum

$CurrentSandX, $CurrentSandY = $StartX, $StartY
do {
    if ($null -eq $OccupiedPositions["$CurrentSandX,$($CurrentSandY+1)"]) {
        $CurrentSandY++
    ...
    } while ( $CurrentSandY -le $AbyssRow )

$OccupiedPositions.Values.Where{$_ -eq "Sand"}.count
# Correct answer = 1406 (24 for testdata)
